...
export default async function FeaturePage({ searchParams }: { searchParams: Promise<{ contest?: string }> }) {
  ...
  const box: React.CSSProperties = {
    border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '22px 24px', background: 'rgba(255,255,255,0.02)',
  }
  const btn: React.CSSProperties = {
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px 24px', borderRadius: 10,
    background: 'linear-gradient(135deg, #4f46e5 0%, #6d28d9 100%)', color: '#fff', fontWeight: 600, fontSize: 14,
    textDecoration: 'none', fontFamily: 'Space Grotesk, sans-serif', boxShadow: '0 0 24px -4px rgba(99,102,241,0.45)',
  }
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#050508', color: '#e4e4e7', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '56px 24px 80px' }}>
        ...
        <h1 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 'clamp(28px, 5vw, 40px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.15, margin: '20px 0 12px', color: '#f4f4f5' }}>
          Get more entries for {picked ? picked.name : 'your AI film contest'}.
        </h1>
        <p style={{ fontSize: 16, lineHeight: 1.65, color: '#a1a1aa', margin: '0 0 32px' }}>
        ...
        <div style={{ ...box, marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#71717a', marginBottom: 14, fontFamily: 'Space Grotesk, sans-serif' }}>
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'grid', gap: 10, fontSize: 15, lineHeight: 1.5 }}>
            ...
            <li>✦&nbsp;&nbsp;A <strong style={{ color: '#a5b4fc' }}>Featured</strong> badge on your listing and in search</li>
        ...
        <div style={{ ...box, marginBottom: 28, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div>
            <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 26, fontWeight: 700, color: '#f4f4f5', letterSpacing: '-0.02em' }}>{PRICE_LABEL}</div>
            <div style={{ fontSize: 13, color: '#71717a' }}>One payment. No subscription. Listing stays free forever.</div>
          </div>
          {picked ? (
            payUrl(picked.id)
        ...
        {!picked && (
          <>
            <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 15, fontWeight: 700, color: '#a1a1aa', margin: '8px 0 12px', letterSpacing: '0.02em' }}>
              Pick your contest
            </h2>
            <div style={{ display: 'grid', gap: 8 }}>
              {open.slice(0, 80).map(c => (
                <Link key={c.id} href={`/feature?contest=${encodeURIComponent(c.id)}`} style={{ ...box, padding: '14px 18px', textDecoration: 'none', color: '#e4e4e7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 14 }}>{c.name} <span style={{ color: '#52525b', fontSize: 12 }}>· {c.organizer}</span></span>
        ...
        <p style={{ fontSize: 13, color: '#52525b', lineHeight: 1.7, marginTop: 36 }}>
          ...
          Questions or corrections: <a href="mailto:hello@aifilmcontests.com" style={{ color: '#71717a' }}>hello@aifilmcontests.com</a>.
        </p>
      </div>
    </main>
  )
